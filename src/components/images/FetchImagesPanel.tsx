...
interface FetchImagesPanelProps {
  hotelName: string;
...
/**
 * Body of the shared image-fetch flow: searches the library, LUX and Google for
 * a hotel, lets the user disambiguate LUX matches or skip to Google, and returns
 * the selected image URLs via onAdd. Rendered as a flex child (scrollable content
 * + pinned footer) so it can live inside either FetchImagesModal's chrome or the
 * case-study ImagePicker's "Find images" tab.
 */
export function FetchImagesPanel({ hotelName, destination, existingUrls, onClose, onAdd, addTargetLabel = 'gallery' }: FetchImagesPanelProps) {
